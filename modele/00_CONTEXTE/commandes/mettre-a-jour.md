...
4. **Vérifier l'intégrité avant de remplacer quoi que ce soit** : lancer `manifeste.py --verifier .` (`python3 …`, ou `py -3 …` sur Windows) depuis le dossier du kit récupéré. Si le kit a été récupéré par `git clone`, retirer d'abord son sous-dossier `.git` avec la commande Python retenue (`import shutil; shutil.rmtree('.git')`), seule suppression définitive autorisée, comme à l'installation. Le script compare chaque fichier à son empreinte SHA-256 inscrite dans `MANIFESTE.txt`. Un fichier manquant ou une empreinte qui ne correspond pas : **arrêter là**, ne rien remplacer, dire lesquels, et proposer de recommencer la récupération autrement. Puis remplacer le contenu de `.kit/` et, dans le dossier documentaire, uniquement les fichiers marqués `kit` du MANIFESTE : `00_CONTEXTE/_scripts/`, `00_CONTEXTE/commandes/`, `.claude/`, `.gemini/commands/`, `.agents/skills/`, et `00_CONTEXTE/SECURITE.md` (copie de `docs/securite.md`).
